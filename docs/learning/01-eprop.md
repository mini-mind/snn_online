...
这类工作还是 e-prop 家族，只是把执行方式改成更事件驱动、更适合稀疏网络和大规模实现。它和本章的关系是“同一条母线上的后续工程化版本”，不是一条新主线。
...
本文默认你已经熟悉神经网络、梯度下降和基本时序建模，但不想在公式推导和生物术语里耗太久。目标是把 e-prop 压缩成一个可迁移研究模块：

```text
入门：知道它解决 recurrent SNN 的时间信用分配
掌握：能写出 eligibility trace × learning signal 的更新式
熟练：能判断 learning signal、trace 时间常数和 neuron model 的取舍
精通：能把它改造成 prediction / reward / teacher modulation 的底层学习规则
```
